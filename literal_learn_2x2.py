...
def generate_hypotheses():
    languages = hypotheses.generate_languages(2)
    lp_pairs = []
...
def main():
    ...
    for _ in range(10):
        post_list1 = simulation(speaker1, 300, priors_unbiased, 5, contexts)
        runs1.append(post_list1)
        post_list2 = simulation(speaker2, 300, priors_unbiased, 3, contexts)
        runs2.append(post_list2)
        post_list3 = simulation(speaker3, 300, priors_unbiased, 4, contexts)
    ...
    with open(filename + '_output.pickle', 'wb') as f:
        pickle.dump(data, f)
    plot_graph("plots/literal_2x2", "Literal speaker + learner: learning 2x2 lexicon and perspective", data)
