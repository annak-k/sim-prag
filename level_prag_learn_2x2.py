...
def produce(system, context):
    ...
    if pragmatic_lvl == 0:
        ...
        if random.random() < noise and num_signals_for_r != 2:
...
def main():
    ...
    if args.spkr == None:
        ...
        plot_graph("plots/2x2_p_lvl" + str(args.p_lvl), plotnames[0], data)
        plot_graph("plots/2x2_p_lvl" + str(args.p_lvl) + '_incorrect', plotnames[1], data_incorrect)
        ...
        for i in range(num_runs):
            if args.old:
                f = open(filename + '_spkr' + str(args.spkr) + '_run' + str(i) +'_2x2.pickle', 'rb')
...
if __name__ == "__main__":  
    ...
    hypotheses, priors, languages = hyp.generate_hypotheses(2, perspectives, p_learner, "egocentric", pragmatic_levels)
    main()